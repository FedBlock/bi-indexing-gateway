...
class IndexingClient {
  ...
  /**
   * 컨트랙트에서 총 요청 개수 조회
   * @param {string} network - 네트워크 이름 (기본값: 'hardhat-local')
   */
  async getTotalRequestCount(network = 'hardhat-local') {
    try {
      // 이더리움 네트워크 연결
      await this.connectEthereumNetwork(network);
      ...
      const contract = new ethers.Contract('0x5FbDB2315678afecb367f032d93F642f64180aa3', contractArtifact.abi, this.ethProvider);
   ...
   * @param {string} network - 네트워크 이름 (기본값: 'hardhat-local')
   */
  async getRequestsInRange(startId, endId, network = 'hardhat-local') {
    try {
      console.log(`🔗 컨트랙트 직접 조회: ${startId} ~ ${endId} (${network})`);
   ...
   * @param {string} network - 네트워크 이름 (기본값: 'hardhat-local')
   */
  async getAllRequestsWithPaging(pageSize = 100, network = 'hardhat-local') {
    try {
      console.log(`🔗 전체 데이터 페이징 조회 시작 (페이지 크기: ${pageSize}, 네트워크: ${network})`);
      
      // 1. 총 개수 조회
      const totalCount = await this.getTotalRequestCount(network);
      ...
      for (let startId = 1; startId <= totalCount; startId += pageSize) {
        ...
        const rangeResult = await this.getRequestsInRange(startId, endId, network);
   ...
   * @param {string} network - 네트워크 이름 (기본값: 'hardhat-local')
   */
  async getFilteredRequestsByPurpose(purpose, pageSize = 100, network = 'hardhat-local') {
    const totalStartTime = Date.now();
    
    try {
      console.log(`🔍 purpose '${purpose}' 필터링 조회 시작 (배치 크기: ${pageSize}, 네트워크: ${network})`);
